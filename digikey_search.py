...
def digikey_search(keywords:str, token_file:str ='.token', limit=10, offset=0):
    ...
    if (expires_by - time_now) < 30:
        ...
        if(refresh_token_expires_by - time_now) < 30:
            logging.error('Refresh token has expired, use digikey_auth to get a new token')
            raise Exception('Refresh token has expired') 
        else:
            # update tokens and token file
            try:
                access_token, refresh_token, expires_in, refresh_token_expires_in = \
                refresh_access_token(client_id=client_id, client_secret=client_secret,
    ...
    if response.status_code != 200:
        logging.error(f'Response status error: {response.status_code} - {response.text}')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--offset', type=int, default=0, help='Index offset for paginate')
    ...
    try:
        ...
        for product in results['Products']:
            logging.debug(f'Manufacturer:{product['Manufacturer']['Name']} - MPN: {product['ManufacturerProductNumber']}')
